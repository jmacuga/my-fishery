...
[dim]Fisherman: {fisherman_name}[/dim]
        ...
        async def run(self):
            ...
            try:
                user_input = await loop.run_in_executor(
                    None, 
                    lambda: console.input("[bold cyan]Enter action number: [/bold cyan]")
                )
        ...
        async def handle_take_fish(self):
            if not self.agent.is_on_fishery:
                console.print("[red]You must be in the fishery first! Use action 1.[/red]")
            ...
            console.print(f"[green]Caught:[/green] {fish_species} (size: {fish_size}, mass: {estimated_mass}kg)")
            logger.info(f"Caught fish: {fish_species}, size: {fish_size}, mass: {estimated_mass}kg")
        ...
        def show_status(self):
            ...
            status_table = Table(title="Fisherman Status", show_header=True, header_style="bold magenta")
            status_table.add_column("Property", style="cyan")
            status_table.add_column("Value", style="green")
            
            status_table.add_row("In fishery", "Yes" if self.agent.is_on_fishery else "No")
            status_table.add_row("Fishes caught", str(len(self.agent.fishes_caught)))
            
            console.print(status_table)
            
            if self.agent.fishes_caught:
                fish_table = Table(title="Caught Fish", show_header=True, header_style="bold blue")
        ...
        async def register_fish_data(
            ...
            if not self.agent.fish_caretaker_jid:
                logger.warning("No fish caretaker JID configured, skipping fish data registration")
                return
    ...
    class HandleIfCanEnterResponseBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if msg:
                ...
                logger.info(f"Received entrance response: {performative} from {msg.sender}")
    ...
    class HandleTakeFishResponseBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if msg:
                ...
                performative = msg.metadata.get("performative", "")
                logger.info(f"Received take fish response: {performative} from {msg.sender}")
    ...
    class HandleFishDataResponseBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if msg:
                protocol = msg.metadata.get("protocol", "")
                if protocol != "register-fish-data":
                    return
                    
                performative = msg.metadata.get("performative", "")
                if performative == "agree" and self.agent.pending_fish_data_registration:
